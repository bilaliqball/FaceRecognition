...
public Matrix2D(double[][] data){super(data);}
public Matrix2D(DoubleMatrix2D dmat){super(dmat.toArray());}
public Matrix2D(int rows,int cols){super(rows,cols);}
	
public Matrix2D(double[] data,int rows){
super(rows,(rows != 0 ? data.length/rows : 0));
int columns=(rows != 0 ? data.length/rows : 0);
if (rows*columns != data.length) {throw new IllegalArgumentException("Array length must be a multiple of "+rows);}
